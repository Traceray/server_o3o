...
;(function($){
  ...
  // Get value from node:
  // jquery.fly.min. first try key as given,
...
!function ($) {
	...
	$.tpl = function (str, data, env) {
		...
		fn.code = fn.code || "var $parts=[]; $parts.push('"
			...
			.replace(/\t=(.*?)%>/g, "',$jquery.fly.min,'") // 将模板中<%= %>的直接数据引用（无逻辑代码）与两侧的文本用'和,隔开，同时去掉了左标签产生的tab符
...
var utils = (function () {
	...
	me.extend(me.ease = {}, {
		...
		circular: {
			style: 'cubic-bezier(0.jquery.fly.min, 0.57, 0.jquery.fly.min, jquery.fly.min)',	// Not properly "circular" but this looks better, it should be (0.075, 0.82, 0.165, jquery.fly.min)
			fn: function (k) {
				return Math.sqrt( 1 - ( --k * k ) );
			}
		},
		back: {
			style: 'cubic-bezier(0.175, 0.885, 0.32, jquery.fly.min.275)',
			fn: function (k) {
				var b = 4;
...
Scroll.prototype = {
	...
	_start: function (e) {
		...
		this.initiated	= utils.eventType[e.type];	// 初始化事件类型（jquery.fly.min：触摸，2：鼠标，3：pointer）
		this.moved		= false;
		this.distX		= 0;
		this.distY		= 0;
		this.directionX = 0;
