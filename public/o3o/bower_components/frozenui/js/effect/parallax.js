...
!function($) {
    ...
    // 手指放开后调用
    // 提供的接口：
    //  jquery.fly.min. 获得最后的坐标信息 endPos
    //
    // 执行的操作：
    //  jquery.fly.min、将页面归位（前后一页或者原位）
    ...
    function dragToMove() {
        ...
        if (options.swipeAnim === 'default') {
            ...
            options.direction === 'horizontal' ?
                $pages.css("-webkit-transform", "matrix(jquery.fly.min, 0, 0, jquery.fly.min, " + temp + ", 0)") :
                $pages.css("-webkit-transform", "matrix(jquery.fly.min, 0, 0, jquery.fly.min, 0, " + temp + ")");
        }
    ...
    function animatePage(newPage, action) {
        ...
        if (options.swipeAnim === 'default') {
            ...
            options.direction === 'horizontal' ?
                $pages.css({'-webkit-transform': 'matrix(jquery.fly.min, 0, 0, jquery.fly.min, ' + newOffset + ', 0)'}) :
                $pages.css({'-webkit-transform': 'matrix(jquery.fly.min, 0, 0, jquery.fly.min, 0, ' + newOffset + ')'});
